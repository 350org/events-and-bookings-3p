...
class Eab_EventsHub {
    ...
    function event_meta_box () {
		...
		$content = apply_filters( 'eab-meta_box-event_meta_box-before', '' );
	...
	private function _meta_box_part_recurring_add ( $event ) {
		...
		if ( in_array( Eab_EventModel::RECURRANCE_WEEKLY, array_keys( $supported_intervals ) ) ) {
			...
			if ( $start_of_week ) {
				for ( $n = 1; $n<=$start_of_week; $n++ ) {
					array_push( $all_weekdays, array_shift( $all_weekdays ) );
				}
			}
    ...
    function handle_post_updated_messages ( $messages ) {
    	...
    	$event = new Eab_EventModel( $post );
    	if ( !$event->is_recurring() ) {
			return $messages; // Normal events don't need this - just recurring
		}

    	$hub_permalink = preg_quote( get_permalink( $post->ID ), '/' );
    	foreach ( $messages['post'] as $idx => $msg ) {
    		if ( !preg_match('/<a .*href=[\'"]' . $hub_permalink . '[\'"]/', $msg ) ) {
				continue;
			}
    ...
    function check_rewrite_rules ( $values ) {
		...
		if ( !post_type_exists( Eab_EventModel::POST_TYPE ) ) {
			return $values;
		}
